...
void write_inode_table(int fd) {
	...
	hello_world.i_mode = EXT2_S_IRUSR
						| EXT2_S_IWUSR
						| EXT2_S_IRGRP
	            		| EXT2_S_IROTH
						| EXT2_S_IFREG;
...
void write_root_dir_block(int fd) {
	...
	struct ext2_dir_entry laf = {0};
	dir_entry_set(laf, LOST_AND_FOUND_INO, "lost+found");
	dir_entry_write(laf, fd);

	bytes_remaining -= laf.rec_len;

	struct ext2_dir_entry hw = {0};
	dir_entry_set(hw, HELLO_WORLD_INO, "hello-world");
	dir_entry_write(hw, fd);

	bytes_remaining -= hw.rec_len;

	struct ext2_dir_entry h = {0};
	dir_entry_set(h, HELLO_INO, "hello");
	dir_entry_write(h, fd);

	bytes_remaining -= h.rec_len;
